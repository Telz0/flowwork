...
export default function CategoriesBeheer() {
  ...
  const save = async () => {
    setSaving(true);
    let savedId = editing;
    const payload = { icon: form.icon, order: form.order, name_nl: form.name, description_nl: form.description };
    if (editing) {
            ...
            <div key={cat.id} className="bg-card border border-border rounded-lg p-2 sm:p-3 flex items-center justify-between gap-2 shadow-sm">
              <div className="flex items-center gap-2 min-w-0">
                ...
                <div className="min-w-0">
                  <p className="font-semibold text-sm truncate">{cat.name_nl || cat.name}</p>
                  {(cat.description_nl || cat.description) && <p className="text-xs text-muted-foreground truncate">{cat.description_nl || cat.description}</p>}
                </div>
              </div>
